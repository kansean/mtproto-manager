...
- **Мониторинг трафика** — агрегированное отслеживание входящего/исходящего трафика со всех контейнеров через Docker API
- **Лимит трафика** — глобальный лимит (ГБ) со снижением скорости (`tc`) при превышении, применяется ко всем контейнерам
...
│   │   └── traffic.py      # Агрегированный мониторинг трафика со всех контейнеров
...
│   └── traffic.json        # Накопленные счётчики трафика (по контейнерам)
...
- **Dashboard** — агрегированный статус прокси ("Running 2/3"), диапазон портов, количество пользователей, трафик, управление запуском
- **Users** — список пользователей с ключами, портами, ссылками `t.me/proxy`, QR-кодами
- **Settings** — домен, IP, базовый порт прокси, Fake TLS домен, proxy tag, лимиты трафика, смена пароля
...
**Особенности:**
- **Агрегация** — трафик со всех контейнеров суммируется в единые счётчики `rx_bytes` / `tx_bytes`
...
- **Лимит трафика** — при превышении заданного лимита (ГБ) применяется ограничение скорости через `tc` (traffic control) **ко всем контейнерам**
...
| 2443-2452 | TCP | MTProto прокси (диапазон для 10 пользователей) | Да |

- Статический IP-адрес или доменное имя
- Для HTTPS необходим домен, направленный A-записью на IP сервера
- Диапазон портов MTProto прокси (по умолчанию 2443-2452) должен быть доступен извне для подключения клиентов Telegram
- Каждый пользователь использует свой порт из диапазона
